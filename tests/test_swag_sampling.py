...
class Test_SWAG_Sampling(unittest.TestCase):
    def test_swag_cov(self, **kwargs):
        ...
        for (module, name), param in zip(
            swag_model.params, model.parameters()
        ):
            ...
            self.assertEqual(
                [swag_model.max_num_models, param.numel()],
                list(cov_mat_sqrt.size()),
            )
        ...
        ) * cov_mat_sqrt.t().matmul(cov_mat_sqrt) + torch.diag(
            sq_mean - mean**2
        )
    ...
    def test_swag_diag(self, **kwargs):
        ...
        for scale in [0.01, 0.1, 0.5, 1.0, 2.0, 5.0]:
            ...
            for _ in range(20):
                ...
                num_in_cr = (
                    ((curr_probs > 0.025) & (curr_probs < 0.975)).float().sum()
                )
