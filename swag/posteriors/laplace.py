...
class KFACLaplace(torch.optim.Optimizer):
    ...
    def __init__(
        ...
        for mod in list(net.modules())[-1:]:
            # a mosidificação feita por Airton em 2022-10-29
            # faz o Laplace olha somente pra ultima camada
            mod_class = mod.__class__.__name__
            if mod_class in ["Linear", "Conv2d"]:
                mod.register_forward_pre_hook(self._save_input)
                mod.register_full_backward_hook(self._save_grad_output)
                params = [mod.weight]
    ...
    def step(self, update_stats=True, update_params=True):
        ...
        for group in self.param_groups:
            ...
            if "BatchNorm" in group["layer_type"] and self.use_batch_norm:
                ...
                weight_inv_chol = torch.linalg.cholesky(weight_hessian)
                ...
                if bias is not None:
                    diag_comp = (
                        ...
                        * self.eps
                        * torch.eye(
                            bias.size(0), device=bias.device, dtype=bias.dtype
                        )
                    )
                    bias_hessian = jacobian(bias.grad, bias) + diag_comp

                    state["b_ic"] = torch.linalg.cholesky(bias_hessian)
    ...
    def _compute_covs(self, group, state):
        ...
        if group["layer_type"] == "Conv2d":
            if not self.sua:
                x = F.unfold(
                    x, mod.kernel_size, padding=mod.padding, stride=mod.stride
                )
            else:
