        ...
        if no_cov_mat is False:
            if int(torch.__version__.split(".")[1]) >= 4:
                module.register_buffer(
                    "%s_cov_mat_sqrt" % name,
                    torch.zeros(max_num_models, data.numel()).cpu(),
                )
            else:
                module.register_buffer(
                    "%s_cov_mat_sqrt" % name,
...
class Laplace(torch.nn.Module):
    def __init__(
        self, base, max_num_models=20, no_cov_mat=False, *args, **kwargs
    ):
    ...
    def sample(self, scale=1.0, cov=False, require_grad=False):
        for module, name in self.params:
            ...
            if not cov:
                ...
                eps = (
                    torch.zeros(cov_mat_sqrt.size(0), 1).normal_().cpu()
    ...
    def export_numpy_params(self):
        ...
        for module, name in self.params:
            ...
            var_list.append(
                module.__getattr__("%s_var" % name).cpu().numpy().ravel()
            )
    ...
    def import_numpy_cov_mat_sqrt(self, w):
        ...
        for (module, name), sq in zip(self.params, w):
            cov_mat_sqrt = module.__getattr__("%s_cov_mat_sqrt" % name)
            cov_mat_sqrt.copy_(
                cov_mat_sqrt.new_tensor(sq.reshape(cov_mat_sqrt.shape))
            )
    ...
    def scale_grid_search(
        ...
        loader,
        criterion,
        logscale_range=torch.arange(-10, 0, 0.5).cpu(),
    ):
