...
def swag_parameters(module, params, no_cov_mat=True):
    for name in list(module._parameters.keys()):
        ...
        module.register_buffer(
            "%s_sq_mean" % name, data.new(data.size()).zero_()
        )
...
class ModSWAG(torch.nn.Module):
    ...
    def sample(
        self, scale=1.0, cov=False, seed=None, block=False, fullrank=True
    ):
        if seed is not None:
    ...
    def sample_fullrank(self, scale, cov, fullrank):
        ...
        for (module, name), sample in zip(self.params, samples_list):
            module.__setattr__(name, sample.cpu())

    def collect_model(self, base_model):
        for (module, name), base_param in zip(
            self.params, base_model.parameters()
        ):
            mean = module.__getattr__("%s_mean" % name)
            sq_mean = module.__getattr__("%s_sq_mean" % name)
    ...
    def export_numpy_params(self, export_cov_mat=False):
        ...
        for module, name in self.params:
            mean_list.append(
                module.__getattr__("%s_mean" % name).cpu().numpy().ravel()
            )
            sq_mean_list.append(
            ...
            if export_cov_mat:
                cov_mat_list.append(
                    module.__getattr__("%s_cov_mat_sqrt" % name)
                    .cpu()
                    .numpy()
                    .ravel()
                )
