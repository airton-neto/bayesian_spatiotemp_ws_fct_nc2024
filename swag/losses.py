...
def adversarial_gaussian_nll(model, input, target):
    epsilon = 0.01
    lossfn = F.gaussian_nll_loss
...
def seg_ale_cross_entropy(model, input, target, num_samples=50, weight=None):
    ...
    for _ in range(num_samples):
        ...
        current_loss, mask = masked_loss(
            sample, target, weight=weight, reduce=False
        )
        total_loss = total_loss + current_loss.exp()
    mean_loss = total_loss / num_samples

    return {
        "loss": mean_loss.log().sum() / mask.sum(),
        "output": mean,
        "scale": scale,
    }
