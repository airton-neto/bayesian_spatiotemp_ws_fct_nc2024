...
class CovarianceSpace(Subspace):
    ...
    def collect_vector(self, vector):
        ...
        self.cov_mat_sqrt = torch.cat(
            (self.cov_mat_sqrt, vector.view(1, -1)), dim=0
        )
        self.rank = torch.min(
            self.rank + 1, torch.as_tensor(self.max_rank)
        ).view(-1)

    def get_space(self):
        return (
            self.cov_mat_sqrt.clone() / (self.cov_mat_sqrt.size(0) - 1) ** 0.5
        )
    ...
    def _load_from_state_dict(
        ...
        self.cov_mat_sqrt = self.cov_mat_sqrt.new_empty(
            (rank, self.cov_mat_sqrt.size()[1])
        )
        super(CovarianceSpace, self)._load_from_state_dict(
            state_dict,
            prefix,
            local_metadata,
            strict,
...
class PCASpace(CovarianceSpace):
    ...
    def get_space(self):
        ...
        _, s, Vt = randomized_svd(
            cov_mat_sqrt_np, n_components=pca_rank, n_iter=5
        )
        ...
        if self.pca_rank == "mle":
            ...
            for rank in range(len(eigs)):
                ...
                m = cov_mat_sqrt_np.shape[1] * rank - rank * (rank + 1) / 2.0
                correction[rank] = 0.5 * m * np.log(cov_mat_sqrt_np.shape[0])
                ll[rank] = _assess_dimension(
                    spectrum=eigs,
                    rank=rank,
            ...
            return torch.FloatTensor(
                s[: self.pca_rank, None] * Vt[: self.pca_rank, :]
            )
        else:
...
class FreqDirSpace(CovarianceSpace):
    ...
    def get_space(self):
        ...
        return (
            self.cov_mat_sqrt[:curr_rank].clone()
            / max(1, self.num_models.item() - 1) ** 0.5
        )
