...
class SWAG(torch.nn.Module):
    def __init__(
        ...
        **kwargs
        ...
        self.num_parameters = sum(
            param.numel() for param in self.base_model.parameters()
        )
    ...
    def to(self, *args, **kwargs):
        ...
        self.subspace.to(
            device=torch.device("cpu"), dtype=dtype, non_blocking=non_blocking
        )
    ...
    def collect_model(self, base_model, *args, **kwargs):
        ...
        w = flatten(
            [param.detach().cpu() for param in base_model.parameters()]
        )
        # first moment
